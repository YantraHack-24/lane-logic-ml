...
async def update_traffic(roads_data: List[RoadData]):
    try:
        current_vehicles = {}
        for road_data in roads_data:
            current_vehicles[road_data.road] = road_data.vehicles
        TrafficManager.api_sync({0: current_vehicles})
        max_lane, green_time = junction.resolve_priority(current_vehicles)
        return {"max_lane": max_lane, "green_time": green_time}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


if __name__ == "__main__":
    uvicorn.run(app, port=8000)  # localhost
